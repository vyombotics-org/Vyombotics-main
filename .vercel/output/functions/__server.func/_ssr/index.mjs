...
function renderErrorPage(error) {
  ...
  const errorStack = error instanceof Error && error.stack ? `<details style="margin-top: 1.5rem; text-align: left;">
...
async function getServerEntry() {
  if (!serverEntryPromise) {
    serverEntryPromise = import("./server-DNxV6PqS.mjs").then((n) => n.s).then(
      (m) => m.default ?? m
    );
  }
